...
1.	项目基于Vue-cli4开发，使用Element-ui搭配部分自定义功能组件实现大部分功能模块开发，通过对项目进行分析和构思，将页面及路由合理拆分

2.	通过路由导航守卫控制访问权限，并在axios请求拦截器添加token，保证用户的数据访问权限；通过多role分控访问权限，在role中嵌套children属性实现多层权限列表。
